<template>
  <UiContent :title="t('menuManageGameServers')" :sub="t('menuManageGameServersInfo')" class="max-w-[700px] mx-auto">
    <UiFlex class="gap-1">
      <USelectMenu v-model="page.size" :options="[5,10,20,50,100]" size="sm"/>

      <SelectDate v-model="page.range.start" :placeholder="t('start')" size="sm" class="max-w-[140px]"/>
      <SelectDate v-model="page.range.end" :placeholder="t('end')" size="sm" class="max-w-[140px] mr-auto"/>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('game/manage/server', page.value)
