<template>
  <UiContent :title="t('menuManageLimitedPayHistory')" :sub="t('menuManageLimitedPayHistoryInfo')">
    <UiFlex class="gap-1" wrap>
      <USelectMenu v-model="page.size" :options="[5,10,20,50,100]"/>

      <UForm :state="page" @submit="page.current = 1, getList()" class="mr-auto">
        <UInput size="sm" v-model="page.user" :placeholder="t('search')" icon="i-bx-search" />
      </UForm>
    ...
    <UCard class="my-2" :ui="{ body: { padding: 'p-0 sm:p-0' } }">
      ...
      >
        ...
        <template #money-data="{ row }">
          <UBadge variant="soft" color="primary">{{ useMoney().toMoney(row.money) }}</UBadge>
        </template>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('limited/pay/manage/history', page.value)
