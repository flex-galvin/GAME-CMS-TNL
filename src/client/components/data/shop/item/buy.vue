<template>
  <UiContent no-dot :title="t('shopItemBuy')" :sub="t('shopItemBuySub')" class="bg-card rounded-2xl p-4">
    <template #more>
    ...
    <div v-if="!load && !!config.maintenance">
      <DataEmpty :text="t('shopMaintenance')" />
    ...
    <UForm :state="state" :validate="validate" @submit="submit" v-if="!load && !config.maintenance">
      ...
      <UFormGroup :label="t('amount')" name="amount" v-if="!!item">
        <UInput v-model="state.amount" :disabled="item.type == 'game_recharge'" type="number" />
      </UFormGroup>

      <UFormGroup :label="t('description')" v-if="!!item && (!!item.description && item.description != '<p></p>')">
        <UCard class="bg-card-box" :ui="{ body: { padding: 'p-4 sm:p-4' } }">
          <UiEditorContent :content="item.description" class="text-xs text-gray-400" />
        </UCard>
      </UFormGroup>

      <UFormGroup :label="t('detailedInformation')" name="info" v-if="!!item && !!level && !!currency">
        <UCard class="bg-card-box" :ui="{ body: { padding: 'p-2 sm:p-2' } }">
          <UiFlex justify="between" class="text-xs sm:text-sm font-semibold p-2 gap-10">
            <UiText color="gray" class="min-w-[100px]">{{ t('item') }}</UiText>
            <UiText align="end">{{ item.name }}</UiText>
          </UiFlex>

          <UiFlex justify="between" class="text-xs sm:text-sm font-semibold p-2 gap-10">
            <UiText color="gray" class="min-w-[100px]">{{ t('unitPrice') }}</UiText>
            <UiText align="end">{{ toMoney(item.price) }}</UiText>
          </UiFlex>
          ...
          <UiFlex justify="between" class="text-xs sm:text-sm font-semibold p-2 gap-10" v-if="!!item.servers">
            <UiText color="gray" class="min-w-[100px]">{{ t('server') }}</UiText>
            <UiText align="end">{{ item.servers.length > 0 ? item.servers.join(' | ') : t('all') }}</UiText>
          </UiFlex>
...
const submit = async () => {
  try {
    buying.value = true
    await useAPI('shop/item/public/buy', state.value)
