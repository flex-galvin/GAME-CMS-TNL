<template>
  <UiContent :title="t('limitedLunarTopReceive')" :sub="t('limitedLunarTopReceiveSub')" class="Lunar rounded-2xl p-4">
    ...
    <UForm :state="state" @submit="submit" >
      <UFormGroup :label="t('receiveReward')" v-if="!!reward">
        <UInput class="lunar-box rounded-2xl" :model-value="`${t('rank')} ${reward.rank}`" readonly />
      </UFormGroup>
...
const submit = async () => {
  try {
    ...
    await useAPI('limited/lunar/public/top/receive', state.value)
