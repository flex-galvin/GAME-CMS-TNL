  ...
  <div>
    ...
    <div class="lunar-box rounded-2xl my-2">
      ...
      <UTable v-model:sort="page.sort" :columns="columns" :rows="list">
        <template #piece-data="{ row }">
          <DataItem :item="{
            item_name: `Mảnh ${row.piece}`,
            item_image: `/images/limited/lunar/piece/${row.piece}.png`,
            type: 'custom'
          }" :amount="1"/>
        </template>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI(`limited/lunar/public/piece/history/collect`, page.value)
