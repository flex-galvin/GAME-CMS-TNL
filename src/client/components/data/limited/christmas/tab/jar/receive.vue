<template>
  <UiContent :title="t('limitedChristmasJarReceive')" :sub="t('limitedChristmasJarReceiveSub')" class="Christmas rounded-2xl p-4">
    ...
    <UForm :state="state" @submit="submit" >
      ...
      <UiFlex justify="end" class="gap-1">
        <UButton class="christmas-btn-2" @click="submit" :loading="loading" v-if="!!isActiveBtn">{{ t('receive') }}</UButton>
...
const submit = async () => {
  try {
    ...
    await useAPI('limited/christmas/public/jar/receive', state.value)
