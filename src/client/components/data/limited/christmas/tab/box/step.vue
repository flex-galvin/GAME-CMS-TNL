<template>
  <UiContent :title="t('limitedChristmasBoxStep')" :sub="t('limitedChristmasBoxStepSub')" class="Christmas rounded-2xl p-4">
    <UiIcon name="i-bx-x" size="8" class="christmas-text absolute top-2 right-2 cursor-pointer z-1" @click="emits('close')" />

    <UiFlex justify="between" class="gap-1 bg-card-box rounded-2xl px-4 py-2 mb-2" v-if="!!user">
      <UiText size="sm" class="christmas-text">{{ t('limitedChristmasBoxOpenTotal') }}</UiText>
      <UiText size="sm FTV text-gradient">{{ user.box }}</UiText>
    </UiFlex>

    <div class="bg-card-box rounded-2xl">
      <UTable :columns="columns" :rows="list">
        <template #step-data="{ row }">
          <UiText class="FTV text-gradient">{{ row.step }}</UiText>
        </template>
        ...
        <template #active-data="{ row }">
          <UiFlex justify="end">
            ...
            <div v-else>
              <button 
                class="christmas-btn px-2 sm:px-4 py-1 rounded-2xl text-[0.65rem] sm:text-sm font-bold text-black" 
                @click="select(row)"
                v-if="user.box >= row.step"
              >
                {{ t('receive') }}
              </button>
    ...
    <UModal v-model="modal" prevent-close>
      <UiContent :title="t('limitedChristmasBoxStepReceive')" :sub="t('limitedChristmasBoxStepReceiveSub')" class="Christmas rounded-2xl p-4">
        <UiIcon name="i-bx-x" size="8" class="christmas-text absolute top-2 right-2 cursor-pointer z-1" @click="modal = false" />

        <UForm :state="state" @submit="submit" >
          <UFormGroup :label="t('need')" v-if="!!reward">
            <UInput :model-value="`${reward.step} ${t('limitedChristmasBoxTimes')}`" readonly />
          </UFormGroup>
...
const list = computed(() => eventData.value.box.reward.sort((a, b) => a.step - b.step))
...
const submit = async () => {
  try {
    loading.value = true
    await useAPI('limited/christmas/public/box/step', state.value)
