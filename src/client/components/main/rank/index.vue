<template>
  <div>
    <DataEmpty :text="t('rankMaintenance')" v-if="!active" />
...
const items = computed(() => {
  const list =  []
  if(!!show.value.level) list.push({ label: t('menuRankLevel'), type: 'level', icon: 'i-icon-park-solid-level' })
  if(!!show.value.power) list.push({ label: t('menuRankPower'), type: 'power', icon: 'i-game-icons-pointy-sword' })
  return list
})
