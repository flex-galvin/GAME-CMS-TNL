    ...
    <UCard class="my-2" :ui="{ body: { padding: 'p-0 sm:p-0' } }">
      ...
      >
        ...
        <template #result-data="{ row }">
          <DataItem :item="{
            name: row.result.item_name,
            image: row.result.item_image,
        ...
        <template #win-data="{ row }">
          <UBadge :color="!!row.win ? 'green' : 'rose'" variant="soft">{{ !!row.win ? t('win') : t('lose') }}</UBadge>
        </template>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('limited/halloween/manage/pumpkin/history', page.value)
