import type { IAuth, IDBAdsLanding } from "~~/types"

export default defineEventHandler(async (event) => {
  try {
    const auth = await getAuth(event) as IAuth
    await checkPermission(event, 'ads.edit')

    const body = await readBody(event)
    const { _id, code, link } = body
    if(!_id || !code || !link) throw 'Dữ liệu đầu vào không hợp lệ'

    const landing = await DB.AdsLanding.findOne({ _id: _id }).select('code') as IDBAdsLanding
    if(!landing) throw 'Landing không tồn tại'

    if(landing.code != code){
      const checkDup = await DB.AdsLanding.findOne({ code: code }).select('_id') as IDBAdsLanding
