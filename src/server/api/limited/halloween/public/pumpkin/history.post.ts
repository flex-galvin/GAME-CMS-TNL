...
export default defineEventHandler(async (event) => {
  try {
    ...
    const list = await DB.LimitedHalloweenPumpkinHistory
    .find(match)
    .populate({ path: 'result', select: 'item_name item_image type' })
    .populate({ path: 'item', select: 'item_name item_image type' })
    .sort(sorting)
    .limit(size)
    .skip((current - 1) * size)

    const total = await DB.LimitedHalloweenPumpkinHistory.count(match)
