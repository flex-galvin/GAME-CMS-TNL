...
import type { IAuth, IDBItem, IDBLimitedHalloween, IDBLimitedHalloweenTopHistory, IDBLimitedHalloweenUser } from "~~/types"
...
export default defineEventHandler(async (event) => {
  try {
    ...
    .select('time top') 
    .populate({ path: 'top.reward.gift.item', select: 'item_id type' })as IDBLimitedHalloween
    if(!config) throw 'Không tìm thấy cấu hình sự kiện Halloween'
    if(!config.time.active) throw 'Sự kiện chưa khai mở'
    if(config.top.reward.length == 0) throw 'Sự kiện chưa cấu hình phần thưởng, vui lòng quay lại sau'
    ...
    // Check User Event
    const userEvent = await DB.LimitedHalloweenUser.findOne({ user: auth._id }).select('_id') as IDBLimitedHalloweenUser
    if(!userEvent) throw 'Bạn không có trong danh sách nhận thưởng'

    // Check History
    const history = await DB.LimitedHalloweenTopHistory.findOne({ user: auth._id }).select('_id') as IDBLimitedHalloweenTopHistory
    if(!!history) throw 'Bạn đã nhận thưởng rồi'

    // Get Rank
    const ranks = await DB.LimitedHalloweenUser.aggregate([
    ...
    if(giftItem.length > 0) await gameSendMail(event, {
      ...
      content: `Nhận thưởng TOP ${rankUser} cống hiến Hũ Bí Ngô trên Web`,
      items: giftItem
    })
    if(Object.keys(giftCurrency).length) await DB.User.updateOne({ _id: auth._id },{ $inc: giftCurrency })
    
    // Save Log
    await DB.LimitedHalloweenTopHistory.create({
      user: auth._id,
      server: server,
      role: role,
      top: rankUser
