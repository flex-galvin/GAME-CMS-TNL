...
import type { IAuth, IDBItem, IDBLimitedChristmas, IDBLimitedChristmasTopHistory, IDBLimitedChristmasUser } from "~~/types"
...
export default defineEventHandler(async (event) => {
  try {
    ...
    .select('time top') 
    .populate({ path: 'top.reward.gift.item', select: 'item_id type' })as IDBLimitedChristmas
    if(!config) throw 'Không tìm thấy cấu hình sự kiện Christmas'
    ...
    // Check User Event
    const userEvent = await DB.LimitedChristmasUser.findOne({ user: auth._id }).select('_id') as IDBLimitedChristmasUser
    if(!userEvent) throw 'Bạn không có trong danh sách tham gia sự kiện'

    // Check History
    const history = await DB.LimitedChristmasTopHistory.findOne({ user: auth._id }).select('_id') as IDBLimitedChristmasTopHistory
    if(!!history) throw 'Bạn đã nhận thưởng rồi'

    // Get Rank
    const ranks = await DB.LimitedChristmasUser.aggregate([
    ...
    if(giftItem.length > 0) await gameSendMail(event, {
      ...
      content: `Nhận thưởng TOP ${rankUser} thắp sáng cây thông trên Web`,
      items: giftItem
    })
    if(Object.keys(giftCurrency).length) await DB.User.updateOne({ _id: auth._id },{ $inc: giftCurrency })
    
    // Save Log
    await DB.LimitedChristmasTopHistory.create({
      user: auth._id,
      server: server,
      role: role,
      top: rankUser
