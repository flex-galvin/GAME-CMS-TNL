...
export default defineEventHandler(async (event) => {
  try {
    ...
    const config = await DB.LimitedChristmas.findOne() as IDBLimitedChristmas
    ...
    config.jar.now = 0
    await DB.LimitedChristmasUser.deleteMany({})
    await DB.LimitedChristmasJarHistory.deleteMany({})
    await DB.LimitedChristmasTopHistory.deleteMany({})
    await DB.LimitedChristmasBoxHistory.deleteMany({})
    await DB.LimitedChristmasBoxStepHistory.deleteMany({})
    await config.save()
