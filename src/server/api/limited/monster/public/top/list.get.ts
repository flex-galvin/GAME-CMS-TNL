import type { IDBLimitedMonster } from "~~/types"

export default defineEventHandler(async (event) => {
  try {
    const config = await DB.LimitedMonster.findOne().select('top') as IDBLimitedMonster
    if(!config) throw 'Không tìm thấy cấu hình sự kiện'

    const list = await DB.LimitedMonsterUser.aggregate([
