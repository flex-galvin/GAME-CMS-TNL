...
export default defineEventHandler(async (event) => {
  try {
    ...
    .select('time jar bonus') 
    .populate({ path: 'jar.reward.gift.item', select: 'item_id type' }) as IDBLimitedLunar
    ...
    // Get Max Step
    const maxStepReward = config.jar.reward.reduce((max, current) => {
      return current.step > max.step ? current : max
    }, config.jar.reward[0])
    const isMaxStep = maxStepReward.step == reward.step

    // Check User Event
    const userEvent = await DB.LimitedLunarUser.findOne({ user: auth._id }).select('point') as IDBLimitedLunarUser
    ...
    // Check History
    const history = await DB.LimitedLunarJarHistory.findOne({ user: auth._id, step: reward.step }).select('_id') as IDBLimitedLunarJarHistory
    ...
    if(giftItem.length > 0) await gameSendMail(event, {
      ...
      content: `Nhận thưởng mốc ${reward.step} góp hũ thần tài trên Web`,
      items: giftItem
    })
    ...
    // Save Log
    await DB.LimitedLunarJarHistory.create({
      user: auth._id,
      server: server,
      role: role,
      step: reward.step
