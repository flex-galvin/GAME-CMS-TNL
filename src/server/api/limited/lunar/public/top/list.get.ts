import type { IDBLimitedLunar } from "~~/types"

export default defineEventHandler(async (event) => {
  try {
    const config = await DB.LimitedLunar.findOne().select('top') as IDBLimitedLunar
    if(!config) throw 'Không tìm thấy cấu hình sự kiện'

    const list = await DB.LimitedLunarUser.aggregate([
