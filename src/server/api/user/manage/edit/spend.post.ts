...
export default defineEventHandler(async (event) => {
  try {
    ...
    if(user.spend.day.coin != day.coin){
      ...
      await logUser(event, user._id, `Tiêu phí <b>ngày</b> được thay đổi từ <b>${user.spend.day.coin.toLocaleString('vi-VN')}</b> thành <b>${day.coin.toLocaleString('vi-VN')}</b> bởi quản trị viên <b>${auth.username}</b> với lý do <b>${reason}</b>`)
    }
    if(user.spend.month.coin != month.coin){
      change.push('tháng')
      update['spend.month.coin'] = parseInt(month.coin)
      await logUser(event, user._id, `Tiêu phí <b>tháng</b> được thay đổi từ <b>${user.spend.month.coin.toLocaleString('vi-VN')}</b> thành <b>${month.coin.toLocaleString('vi-VN')}</b> bởi quản trị viên <b>${auth.username}</b> với lý do <b>${reason}</b>`)
    }
    if(user.spend.total.coin != total.coin){
      change.push('tổng')
      update['spend.total.coin'] = parseInt(total.coin)
      await logUser(event, user._id, `Tiêu phí <b>tổng</b> được thay đổi từ <b>${user.spend.total.coin.toLocaleString('vi-VN')}</b> thành <b>${total.coin.toLocaleString('vi-VN')}</b> bởi quản trị viên <b>${auth.username}</b> với lý do <b>${reason}</b>`)
    }

    if(change.length > 0){
      await DB.User.updateOne({ _id: _id }, update)
      await logAdmin(event, `Sửa dữ liệu tiêu phí <b>${change.join(', ')}</b> của tài khoản <b>${user.username}</b> với lý do <b>${reason}</b>`)
    }
