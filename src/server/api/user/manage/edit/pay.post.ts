...
export default defineEventHandler(async (event) => {
  try {
    ...
    if(user.pay.day.money != day.money){
      ...
      await logUser(event, user._id, `Tích nạp <b>ngày</b> được thay đổi từ <b>${user.pay.day.money.toLocaleString('vi-VN')}</b> thành <b>${day.money.toLocaleString('vi-VN')}</b> bởi quản trị viên <b>${auth.username}</b> với lý do <b>${reason}</b>`)
    }
    if(user.pay.month.money != month.money){
      change.push('tháng')
      update['pay.month.money'] = parseInt(month.money)
      await logUser(event, user._id, `Tích nạp <b>tháng</b> được thay đổi từ <b>${user.pay.month.money.toLocaleString('vi-VN')}</b> thành <b>${month.money.toLocaleString('vi-VN')}</b> bởi quản trị viên <b>${auth.username}</b> với lý do <b>${reason}</b>`)
    }
    if(user.pay.total.money != total.money){
      change.push('tổng')
      update['pay.total.money'] = parseInt(total.money)
      await logUser(event, user._id, `Tích nạp <b>tổng</b> được thay đổi từ <b>${user.pay.total.money.toLocaleString('vi-VN')}</b> thành <b>${total.money.toLocaleString('vi-VN')}</b> bởi quản trị viên <b>${auth.username}</b> với lý do <b>${reason}</b>`)
    }

    if(change.length > 0){
      await DB.User.updateOne({ _id: _id }, update)
      await logAdmin(event, `Sửa dữ liệu tích nạp <b>${change.join(', ')}</b> của tài khoản <b>${user.username}</b> với lý do <b>${reason}</b>`)
    }
