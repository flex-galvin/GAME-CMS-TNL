...
import { IDBConfig, IDBUser, IDBAdsLanding, IDBLogUserIP, IDBAdsFrom } from '~~/types'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { username, password, landing, captcha } = await readBody(event)
    ...
    await logUser(event, user._id, `Đăng nhập bằng Landing <b>${landingData.code}</b> với IP <b>${IP}</b>`)
