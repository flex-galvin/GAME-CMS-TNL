...
export default defineEventHandler(async (event) => {
  try {
    ...
    if(eventData.need != need){
      const getByNeed = await DB.Event.findOne({ need: need, type: eventData.type }).select('_id') as IDBEvent
    ...
    await logAdmin(event, `Sửa mốc <b>${eventData.need}</b> cho sự kiện <b>${eventConfig.name}</b>`)
